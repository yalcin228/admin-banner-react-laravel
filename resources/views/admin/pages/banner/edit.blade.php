...
@section('title','Banner Redaktəsi')
@section('content')
...
{{--                                              class="form-control @error('ads') is-invalid @enderror">{!! $banner->ads !!}</textarea>--}}
                                    <input type="text" name="ads" class="form-control" value="{{$banner->ads}}">
                                    @error('ads')
                                    <span class="">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select site @error('site_id') is-invalid @enderror" required name="site_id"
                                            ...
                                            @foreach ($sites as $site)
                                                <option
                                                    value="{{ $site->id }}" {{$site->id == $banner->site_id ? 'selected':''}}>{{$site->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('site_id')
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('category_id') is-invalid @enderror" required name="category_id" id="category">
                                            <option value="" selected="selected">Seçin</option>
                                            @foreach ($categories as $category)
                                                <option
                                                    value="{{ $category->id }}" {{$category->id == $banner->category_id ? 'selected':'' }}>{{$loop->iteration}}
                                ...
                                <div class="col-md-12">
                                    ...
                                    'attrs' => 'required','selected' => $banner->status])
                                </div>
